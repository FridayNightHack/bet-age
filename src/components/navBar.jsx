      ...
      <header className="w-full h-basic text-white fixed backdrop-blur-sm font-basic z-10">
        <div className="container-wide flex items-center justify-between border-b-2 border-dark-gray">
          ...
          <div className="flex items-center gap-4 ">
            <button className="relative flex justify-between items-center" onClick={showSide}>
              <FaTicket size={25} />
              {betSlip.length > 0 && (
                <span
                  className={`absolute -top-1 -right-1 bg-green-600 w-3 h-3 rounded-full block animate-pulse`}></span>
              )}
            </button>
            <time dateTime={new Date().toISOString().split('T')[0]} className="max-sm:hidden">
      ...
      <aside
        ...
        <div className="w-full relative flex flex-row items-center justify-start gap-x-3">
          ...
          <HiX
            ...
            onClick={() => setIsClicked(!isClicked)}></HiX>
        </div>
          ...
          {betSlip.length > 0 ? (
            <>
              <ul className="max-h-[16.75rem] flex flex-col gap-y-2 overflow-y-scroll overflow-hidden font-segoe mb-5">
                {/* Список ставок */}
                {betSlip.map(
                  ({ matchId, team1, team2, localized, marketName, betType, odds }, index) => (
                    <li
                      ...
                      <div className="flex flex-row items-start border-r border-dark-gray px-2 py-2 gap-x-1">
                        ...
                        <div className="flex flex-col gap-y-3">
                          ...
                          <span className="text-sm text-gray-300">{betType}</span>
                        </div>
                      </div>
              ...
              <div className=" bg-gray-800 text-white p-3 rounded-lg text-center">
                <strong className="text-sm opacity-90 uppercase">Umumy koeffitsient</strong>
                <div className="text-2xl font-bold mt-1">x{totalOdds.toFixed(2)}</div>
              </div>
              <div className="mt-4">
                <label className="block text-sm font-medium text-light mb-2">Goýum(TMT)</label>
                <input
                  type="number"
                  placeholder="100"
                  value={stake}
              ...
              <div className="bg-gradient-to-r from-green-500 to-green-600 text-white p-4 rounded-lg mt-4 text-center shadow-lg">
                ...
                <div className="text-2xl font-bold mt-1">{potentialWin} TMT</div>
              </div>
              <div className="mt-4">
                <button
                  onClick={() => {
                    stake && parseFloat(stake) > 0 ? setShowForm(true) : alert('Введите сумму');
                  }}
